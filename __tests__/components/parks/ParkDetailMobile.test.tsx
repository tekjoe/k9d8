...
import ParkDetailMobile from '../../../src/components/parks/ParkDetailMobile';
...
describe('ParkDetailMobile', () => {
  ...
  it('renders without crashing', async () => {
    const { toJSON } = render(<ParkDetailMobile slugOrId="park-1" />);
    await waitFor(() => {
  ...
  it('displays park name after loading', async () => {
    const { getByText } = render(<ParkDetailMobile slugOrId="park-1" />);
    await waitFor(() => {
      expect(getByText('Test Park')).toBeTruthy();
    });
  });

  it('shows error state when park not found', async () => {
  ...
  it('displays About This Park section', async () => {
    const { getByText } = render(<ParkDetailMobile slugOrId="park-1" />);
    await waitFor(() => {
