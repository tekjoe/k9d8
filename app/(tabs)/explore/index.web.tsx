...
export default function DesktopExploreScreen() {
    ...
    return (
      ...
      <View style={{ flex: 1, backgroundColor: '#F5F4F1' }}>
        ...
        <View style={{ flex: 1, position: 'relative' }}>
          ...
          {searchQuery.trim().length > 0 && mobileShowMap && (
            <View
              style={{
                position: 'absolute',
                top: 0,
            ...
            >
              ...
              >
                {filteredParks.length > 0 ? (
                  <>
                    <Text style={{ fontSize: 13, fontWeight: '600', color: '#9C9B99', marginBottom: 12 }}>
                      {allFilteredParks.length} {allFilteredParks.length === 1 ? 'result' : 'results'}
                    </Text>
                    {filteredParks.map(({ park, distanceKm }) => (
                      <ParkListItem
                        key={park.id}
  ...
  return (
    ...
    <View className="flex-1 flex-row bg-background">
      ...
      {/* Parks List Panel */}
      <View className="bg-white border-r border-gray-200 flex-col" style={{ width: 360, minWidth: 360, maxWidth: 360 }}>
