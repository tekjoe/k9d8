...
import { View, Text, Pressable, ScrollView, TextInput, ActivityIndicator, Image, useWindowDimensions, Platform } from 'react-native';
...
export default function DogParkSlugPage() {
  ...
  if (slugParts.length >= 2) {
    ...
    const parkSlug = slugParts.slice(1).join('/');
  ...
  // Single slug handling
  if (slugParts.length === 1) {
    const singleSlug = slugParts[0];

    // On web, single slug = state listing page (e.g. /dog-parks/minnesota)
    if (Platform.OS === 'web' && STATE_SLUGS.includes(singleSlug)) {
      return <StateDogParksPage stateSlug={singleSlug} />;
    }

    // Native: treat as park ID (auth required)
    if (!session) {
      return (
    ...
    return <ParkDetailAuth slugOrId={singleSlug} />;
  }

  // Unrecognized route
  return (
    <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: '#F5F4F1', padding: 24, gap: 16 }}>
      <Text style={{ fontSize: 16, color: '#6D6C6A' }}>This page is not available.</Text>
