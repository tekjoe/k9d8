...
export default function DogParkSlugPage() {
  ...
  // Park detail: slug ends with 8-char hex
  if (extractShortIdFromSlug(slug || '')) {
    return <ParkDetailMobile slugOrId={slug!} />;
  }
...
function StateDogParksPage({ stateSlug }: { stateSlug: string }) {
  ...
  // Convert slug to state name: "minnesota" → "Minnesota"
    ...
    <ScrollView style={{ flex: 1, backgroundColor: '#F5F4F1' }} contentContainerStyle={{ minHeight: '100%' }}>
      ...
      <Container>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, paddingVertical: 8 }}>
          ...
          <Text style={{ fontSize: 14, color: '#6D6C6A' }}>{stateName}</Text>
        </View>
      </Container>
          ...
          {loading ? (
            ...
            <View style={{ alignItems: 'center', paddingVertical: 60, gap: 16 }}>
              ...
              <Text style={{ fontSize: 16, color: '#6D6C6A', textAlign: 'center' }}>
                No parks found in {stateName}. Try browsing our full directory.
              </Text>
              <Pressable
